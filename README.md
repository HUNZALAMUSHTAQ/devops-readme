...
- Import YAML File into Azure pipelines
...
# Variable 'AWS_TOOL_ARTIFACT_GUID' was defined in the Variables tab
# Variable 'AWS.Region' was defined in the Variables tab
variables:
...
resources:
  ...
  - repository: self
    type: git
    ref: refs/heads/develop-pipeline
jobs:
- job: Job_1
  ...
  - checkout: self
    ...
    fetchTags: false
  - task: PowerShell@2
    displayName: AWS Credential Provider
    enabled: False
    inputs:
      targetType: inline
      script: "$awsAccessKeyId = \"$(AWS_ACCESS_KEY_ID)\"\n    $awsSecretAccessKey = \"$(AWS_SECRET_ACCESS_KEY)\"\n    $toolkit_artifact_guid=\"$(AWS_TOOL_ARTIFACT_GUID)\"\n\n    $awsProfileDirectory = \"$env:USERPROFILE\\.aws\"\n    if (-not (Test-Path -Path $awsProfileDirectory)) {\n        New-Item -Path $awsProfileDirectory -ItemType Directory | Out-Null\n    }\n\n    $awsCredentialsFilePath = \"$awsProfileDirectory\\credentials\"\n    \n    $awsCredentials = @(\n        \"[healthquest-s3]\",\n        \"aws_access_key_id = $awsAccessKeyId\",\n        \"aws_secret_access_key = $awsSecretAccessKey\",\n        \"toolkit_artifact_guid=$toolkit_artifact_guid\"\n    ) -join \"`n\"\n\n    $awsCredentials | Out-File -FilePath $awsCredentialsFilePath -Encoding UTF8\n\n # Echo the path of the credentials file\n    Write-Host \"Credentials file path: $awsCredentialsFilePath\"\n    \n    # Echo the contents of the credentials file\n    Write-Host \"Credentials file contents:\"\n    Get-Content -Path $awsCredentialsFilePath"
  - task: DotNetCoreCLI@2
    displayName: Restore
    inputs:
      command: restore
      projects: $(BuildParameters.RestoreBuildProjects)
      feedRestore: 48af52c3-c4ed-4bd4-aaa6-911ceea2f37c/29103782-1642-4a6a-a887-6f4c465c436b
  - task: DotNetCoreCLI@2
    displayName: Build
    timeoutInMinutes: 120
    inputs:
  ...
  - task: pekspro.pekspro-efcore-migration-script-generator.efcore-migration-script-generator.efcore-migration-script-generator-task@1
    ...
    inputs:
      projectpath: HealthQuest.Data/HealthQuest.Data.csproj
      databasecontexts: HealthQuestDBContext
      startupprojectpath: HealthQuest.API/HealthQuest.API.csproj
      build: false
